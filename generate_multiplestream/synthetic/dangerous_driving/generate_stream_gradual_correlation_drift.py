...
'''
gradual drift transition function probability

time_interval: [drift_point-drift_length//2, drift_point + drift_length//2]
return p: the probability that the sample follows the distribution after gradual concept drift
'''
def drift_transition_prob(time, drift_point, drift_length):
    return 1/(1+math.exp(-4*(time-drift_point)/drift_length))
...
save_labelled_stream(stream_1, 'log/synthetic/dangerous_driving/gradual_correlation_drift/stream_1.csv')
...
save_labelled_stream(stream_2,'log/synthetic/dangerous_driving/gradual_correlation_drift/stream_2.csv')
...
for index in range(stream_3.shape[0]):
    ...
    if index < drift_point - drift_length//2:
        if instance_label[1] == 0:
            stream_3[index,:] = np.array([np.random.uniform(3,6),1])
        else:
            stream_3[index,:] = np.array([np.random.uniform(0,3),0])
            
    # gradual drift occurs
    elif drift_point - drift_length//2 <= index < drift_point + drift_length//2:
        
        p = drift_transition_prob(index,drift_point,drift_length)
        rand = np.random.rand()
...
save_labelled_stream(stream_3,'log/synthetic/dangerous_driving/gradual_correlation_drift/stream_3.csv')
...
save_labelled_stream(stream_all,'log/synthetic/dangerous_driving/gradual_correlation_drift/stream_all.csv')
