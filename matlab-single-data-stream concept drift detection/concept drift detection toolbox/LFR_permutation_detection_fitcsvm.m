...
for i=2:size(data,2)
    ...
    if (detect_tpr+detect_tnr+detect_ppv+detect_npv>0) && (i-pre_detect_time>distance_threshold) ...
        ...
        if i-pre_permutation_time >= permutation_window
        ...
        elseif (i-warn_time) < num_least_relearn && (warn_time + num_least_relearn) <= size(data,2)
            % SVMModel = svmtrain(y(warn_time:(warn_time+num_least_relearn)), X(warn_time:(warn_time+num_least_relearn),:),'-t 2 -c 100 -g 1 -q');
             SVMModel = fitcsvm(X(warn_time:(warn_time+num_least_relearn),:), (y(warn_time:(warn_time+num_least_relearn))), 'KernelFunction','linear');
            num_wait_steps = warn_time + num_least_relearn - i;
        elseif (i-warn_time) < num_least_relearn && (warn_time + num_least_relearn) > size(data,2)
            % SVMModel = svmtrain(y(warn_time:end),X((warn_time:end),:),'-t 2 -c 100 -g 1 -q');
             SVMModel = fitcsvm(X((warn_time:end),:), (y(warn_time:end)), 'KernelFunction','linear');
