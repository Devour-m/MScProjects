function [detect_time,data,tpr,tnr,ppv,npv] = CBHCDT_f_LFR_permutation_detection(TrainingData,TestingData,eta,...
    num_least_relearn,mode,table_num,distance_threshold,permutation_window, classifier)
% load boundtable BT
if table_num == 1
...
elseif strcmpi(mode,'real') == 1
    ...
    % SVMModel = fitcsvm(TrainingData(:,1:end-1), TrainingData(:,end),'KernelFunction','linear');
    
     switch classifier 
        case 'SVM_linear'
            Mdl = fitcsvm(TrainingData(:,1:end-1), (TrainingData(:,end)), 'KernelFunction','linear');
        case 'SVM_rbf'
            Mdl = fitcsvm(TrainingData(:,1:end-1), (TrainingData(:,end)), 'KernelFunction','rbf');
        case 'Tree'
            Mdl = fitctree(TrainingData(:,1:end-1), (TrainingData(:,end)));
    end
...
for i=2:size(data,2)
    ...
    if (detect_tpr+detect_tnr+detect_ppv+detect_npv>0) && (i-pre_detect_time>distance_threshold) ...
        ...
        test_value = CBHCDT_f_permutation_test(PermutationData,permutation_window,1000,0,0.2,classifier);
        ...
        if (i-warn_time) >= num_least_relearn
            ...
            % SVMModel = fitcsvm(X(warn_time:i,:), y(warn_time:i),'KernelFunction','linear');
        ...
        elseif (i-warn_time) < num_least_relearn && (warn_time + num_least_relearn) <= size(data,2)
            ...
            %    '-t 2 -c 100 -g 1 -q');
             % Mdl = fitcsvm(X(warn_time:(warn_time+num_least_relearn),:), ...
            ...
            switch classifier
                ...
                case 'SVM_rbf'
                    Mdl = fitcsvm(X(warn_time:(warn_time+num_least_relearn),:), ...
                        ...
                        'KernelFunction','rbf');
                case 'Tree'
                    Mdl = fitctree(X(warn_time:(warn_time+num_least_relearn),:), y(warn_time:(warn_time+num_least_relearn)));
            end
        ...
        elseif (i-warn_time) < num_least_relearn && (warn_time + num_least_relearn) > size(data,2)
            ...
            % SVMModel = fitcsvm(X((warn_time:end),:), ...
            %    y(warn_time:end),...
            %    'KernelFunction','linear');
            
            switch classifier
                case 'SVM_linear'
                    Mdl = fitcsvm(X((warn_time:end),:), ...
                        y(warn_time:end),...
                        'KernelFunction','linear');
                case 'SVM_rbf'
                    Mdl = fitcsvm(X(warn_time:(warn_time+num_least_relearn),:), ...
                        ...
                        'KernelFunction','rbf');
                case 'Tree'
                    Mdl = fitctree(X(warn_time:(warn_time+num_least_relearn),:), y(warn_time:end));
            end
           num_wait_steps = size(data,2) - i;  
        end
        end
